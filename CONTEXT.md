...
The vocabulary of this codebase.
<!-- feat:if backend -->
Three different words here mean "a unit of the system" and they are not
interchangeable; much of this glossary exists to keep them apart.
<!-- feat:end -->
...
switch.
_Avoid_: feature flag, toggle, option — each implies a decision made at run time
...
_Avoid_: deployment, profile (`profile` is a Compose term, used here only for
the `app` profile)
