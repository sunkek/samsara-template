...
if [ -z "$DEST" ] || [ "$DEST" = "." ]; then
  ...
  rm -rf "$TARGET/.git" \
         "$TARGET/node_modules" "$TARGET/services/frontend/node_modules" \
...
# a broken fork.
if grep -qE '^module[[:space:]]+[^[:space:]]+[[:space:]]+[^[:space:]]' services/backend/go.mod; then
  echo "ERROR: services/backend/go.mod module path contains whitespace after rename:" >&2
  grep -n '^module' services/backend/go.mod >&2
  exit 1
fi
